...
def compute_batch_loss(batch, n_rotations_train: int = 0):
    ...
    # Build edges once; rotations are global => graph structure and distances unchanged
    edge_index = build_block_complete_graph(b)
    edge_index, _ = remove_self_loops(edge_index)

    # --- No augmentation case: exactly what you had before ---
    if n_rotations_train <= 0:
...
for epoch in range(start_epoch, NUM_EPOCHS + 1):
    ...
    for batch in tqdm(train_loader):
        model.train()
        # use rotationally-augmented loss during training
    ...
    # Validation: keep it *unaugmented* so you can see true generalisation
    val_loss = evaluate(val_loader)

    # Rotational robustness check (you already had this)
